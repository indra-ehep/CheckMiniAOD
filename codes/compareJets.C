...
void compareJets(int index = 0, const char *infile = "/home/indra/temp/a532f04b-e2a8-4e53-b079-6e06945eb6b0.root") {
  ...
  TFile *fgraphs = new TFile("root_files/fitresults_save.root");
  ...
  TH1D *hETICLByGen = new TH1D("hETICLByGen","hETICLByGen", 600, -0.5, 5.5);
  ...
  for(int ibin=0;ibin<nEbins;ibin++){
    hEResoP[ibin] = new TH1D(Form("hEResoP_%d",ibin), Form("hEResoP_%d (%4.1f GeV)",ibin,hEBin->GetXaxis()->GetBinCenter(ibin+1)), 200, -100.0, 100.0);
    hEDiff[ibin] = new TH1D(Form("hEDiff_%d",ibin), Form("hEDiff_%d (%4.1f GeV)",ibin,hEBin->GetXaxis()->GetBinCenter(ibin+1)), 200, -100.0, 100.0);
  }
  ...
  TGraph *grCalib[nJetEtaBins] ; 
  for(int ieta=0;ieta<nJetEtaBins;ieta++)
    grCalib[ieta] = (TGraph *)fgraphs->Get(Form("graph_%d",ieta));
  ...
  for( ev.toBegin(); ! ev.atEnd() ; ++ev) {
	...
	if(delEtaDelPhi<0.1) {
	 ...
	 ietabin -= 1;
	 totE *= (0.975*grCalib[ietabin]->Eval(totE));
       }

       double eratio = (totE/(*genjets)[ipg].energy()) ;
       double ediff = ((*genjets)[ipg].energy() - totE) ;
       hETICLByGen->Fill( eratio );
       if( (*genjets)[ipg].energy()<maxE and (TMath::Abs((*genjets)[ipg].eta())>1.7 and TMath::Abs((*genjets)[ipg].eta())<2.9) ){
       //if( totE<maxE and (TMath::Abs(ticlEta)>1.7 and TMath::Abs(ticlEta)<2.9)){
       	 hETICLGenDiff->Fill( ediff  );
       	 int ibin = hEBin->GetXaxis()->FindBin((*genjets)[ipg].energy()) - 1;
	 //int ibin = hEBin->GetXaxis()->FindBin(totE) - 1;
	 if(eratio>0.5 and eratio<1.5){
